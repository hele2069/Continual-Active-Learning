'''
From https://github.com/cjshui/WAAL
'''
import numpy as np
import torch
...
class WAAL:
    ...
    def __init__(self,X,Y, idx_lb, net_fea, net_clf, net_dis, train_handler, test_handler, args, X_val, Y_val):
        """
    ...
    def train(self, alpha, total_epoch):
        ...
        self.fea = self.net_fea('VGG16').to(self.device)
    ...
    def predict(self,X,Y):
        ...
        loader_te = DataLoader(self.test_handler(X, Y, transform=self.args['transform']),
    ...
    def validation(self):
        loader_val = DataLoader(self.handler(self.X_val, self.Y_val, transform=self.args['transform']), batch_size=1024, shuffle=False)
        self.clf.eval()
        correct = 0
        with torch.no_grad():
