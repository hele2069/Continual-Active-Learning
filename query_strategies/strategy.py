...
class Strategy(nn.Module):
    def __init__(self, X, Y, idxs_lb, net, handler, args, seeds=None, X_val=None, Y_val=None):
        ...
        # EWC
        self.use_ewc = False # switch for using ewc or lwf
    ...
    def _train(self, epoch, loader_tr, optimizer, ins_weight=0, is_noise=False, loader_bottom=None):
        self.clf.train()
        for batch_idx, (x, y, idxs) in enumerate(loader_tr):
            x, y = x.to(self.device), y.to(self.device)
            optimizer.zero_grad()

            if not is_noise: # if not training with noise
                out = self.clf(x)
                y_onehot = torch.FloatTensor(len(y), self.args.n_class).to(self.device)
                y_onehot.zero_()
                y_onehot.scatter_(1, y[:, None], 1)
                loss = F.cross_entropy(out, y)
                if self.round > 0 and self.continual: # ewc loss for continual learning
                    '''
    ...
    def train(self, use_history=False):
        ...
        for epoch in range(1, n_epoch + 1):
            ...
            if not use_history:
                ...
                print('history')
                self._train(epoch, loader_tr, optimizer, is_noise=True)
    ...
    # hyper parameters heuristics (lambda from big to small)
    # why fisher info small at first big later
    # 实现：每一轮只更新最不重要的X% parameter （不同百分比比例scale的变化）fisher
    def estimate_fisher(self):
        ...
        if self.online_ewc: idxs_train = self.new_selected
        else: idxs_train = np.arange(self.n_pool)[self.idxs_lb]
        ...
        for index, (x, y, idxs) in enumerate(loader_tr):
            if self.fisher_n is not None and index >= self.fisher_n:
                break
            x = x.to(self.device)
            output = self.clf(x)
            if self.emp_FI:
    ...
    def estimate_ntk(self, X, Y, embed=False):
        ...
        for index, (x, y, idxs) in enumerate(loader_te):
            ...
            if not expectation: # ? 
                ...
                """
                for name, p in self.clf.classifier.named_parameters():
                    print(name)
                print('--------------------')
                for name, p in self.clf.named_parameters(recurse=True):
                    print(name)                
                print('--------------------')
                for name, p in self.clf.named_parameters():
                    print(name)                
                print('--------------------')
                print(self.z)
                """
                ...
                """
                feature.weight - feature__weight
                feature.bias - feature__bias
                classifier.weight - classifier__weight
                classifier.bias - classifier__bias
                """
                # for n, p in list(self.clf.classifier.named_parameters()):
                for n, p in list(self.clf.named_parameters()):
                    ...
                    if p.grad_batch is not None:
                        ...
                        cb_raw = p_grad.flatten(start_dim=1) / deno # try setting to 1 (oct.31)
                ...
                for label in range(self.args.n_class):
                    ...
                    # for n, p in self.clf.classifier.named_parameters():
                    for n, p in self.clf.named_parameters():
                        n = n.replace('.', '__')
                        # n = 'classifier__' + n
                        if p.grad_batch is not None:
    ...
    def estimate_z(self):
        ...
        for index, (x, y, idxs) in enumerate(loader_tr):
            ...
            for n, p in list(self.clf.named_parameters()):
                if p.requires_grad:
                    ...
                    if hasattr(p, 'grad_batch') and p.grad_batch is not None:
                        p_grad = p.grad_batch.detach()
                        a = torch.sum((p_grad ** 2).flatten(start_dim=1), 0)
                        b = torch.stack([torch.sum((p_grad ** 2).flatten(start_dim=1)[y == i], 0) 
                                        for i in range(10)], 0)
                        if n not in tmp_z:
                            tmp_z[n] = a
                            tmp_z_cls[n] = b
        ...
        for n, p in list(self.clf.named_parameters()):
            if p.requires_grad:
                ...
                mean = np.mean((1 / self.z[n]).cpu().numpy())
                sd = np.std((1 / self.z[n]).cpu().numpy())
                if n +'_mean' not in self.z_history:
                    self.z_history[n+'_mean'] = [mean]
                    self.z_history[n+'_std'] = [sd]
                elif n +'_mean' in self.z_history:
                    self.z_history[n+'_mean'].append(mean)
                    self.z_history[n+'_std'].append(sd)
    ...
    # only accounts last gradient layer (compared to NTK)
    # test when NTK uses last layer gradient + Z 
    # cv 
    def get_grad_embedding(self, X, Y):
        model = self.clf
